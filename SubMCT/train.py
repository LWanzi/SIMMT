...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=2)#900
    ...
    for id, name in zip(ids, img_names):
        ...
        adata, patchs, centers = preprocess_CosMx_data(opt,id,name)
        ...
        # define model
        # model = Train10XSingleAddCL(adata,patchs1,centers,hidden_dims=[512,32], n_epochs=opt.epochs,
        #                             lr=opt.lr, random_seed=opt.seed, device=device,save_path=sp, ncluster=opt.ncluster, repeat=1, use_combine=1)
        model = TrainNanosingleAddcl(adata, patchs, centers, hidden_dims=[32], n_epochs=opt.epochs, lr=opt.lr,
                                     random_seed=opt.seed, save_path=sp, ncluster=opt.ncluster, repeat=1)#128
        # train model
        adata = model.train()
