...
from loss import CL_loss,CL_loss_SCprotein,CL_loss_BLEEP,sce_loss
...
class TrainNanosingleAddcl:
    def __init__(self,
        ...
        self.adj = preprocess_adj(self.data.edge_index, self.num_nodes)  # convert edge_index to adj,then Laplacian normalization
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, self.n_epochs + 1)):
            ...
            gz, iz, cz,gout, iout,cout,gz_contra, iz_contra,x_init,x_rec = self.model(self.data.x, self.img.x, self.adj,self.data.edge_index, self.patchs, self.centers,device=self.device)
            ...
            cl_loss =CL_loss_SCprotein(gz_contra, iz_contra) #CL_loss_SCprotein
